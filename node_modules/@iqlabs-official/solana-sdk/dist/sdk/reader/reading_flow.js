...
async function readSession(sessionPubkey, readOption, speed, onProgress) {
    ...
    return (0, reading_methods_1.readSessionResult)(sessionPubkey, readOption, speed, onProgress);
}
async function readLinkedListFromTail(tailTx, readOption, onProgress, expectedTotalChunks) {
    const connection = (0, connection_helper_1.getReaderConnection)(readOption.freshness);
    ...
    return (0, reading_methods_1.readLinkedListResult)(tailTx, readOption, onProgress, expectedTotalChunks);
}
async function readUserInventoryCodeInFromTx(tx, speed, onProgress) {
    const blockTime = tx.blockTime;
    const { onChainPath, metadata, inlineData } = (0, reader_utils_1.extractCodeInPayload)(tx);
    let totalChunks;
    try {
    ...
    if (kind === "session") {
        const { result } = await readSession(onChainPath, readOption, speed, onProgress);
        return { metadata, data: result };
    }
    const { result } = await readLinkedListFromTail(onChainPath, readOption, onProgress, totalChunks);
    return { metadata, data: result };
}
async function readUserState(userPubkey) {
    const connection = (0, connection_helper_1.getConnection)();
