...
function buildReallocIxIfNeeded(builder, payer, target, accountData) {
    ...
    const usedBytes = 8 + 32
        ...
        + 4 + decoded.id.length;
    if (accountData.length - usedBytes >= REALLOC_THRESHOLD)
        return null;
    return (0, contract_1.reallocAccountInstruction)(builder, { payer, target, system_program: web3_js_1.SystemProgram.programId }, { new_size: new anchor_1.BN(accountData.length + REALLOC_EXTRA) });
}
async function createTable(connection, signer, dbRootId, tableSeed, tableName, columnNames, idCol, extKeys, gate, writers) {
