...
import {timeAgo, shortenSig, truncate} from "../../utils/format";
import {
...
const uploadToPlazaFlow = async (service: FileShareService, speed: SpeedProfile) => {
    ...
    console.log(
        `            (${shortenSig(service.signer.publicKey.toBase58(), 4)}) is stamped on chain forever.`,
    );
...
const createPlazaFolderFlow = async (
    ...
    try {
        ...
        if (!result.created) {
            ...
            logSuccess(`created. tx: ${shortenSig(result.signature, 6)}`);
        }
    ...
    while (true) {
        ...
        const idx = await selectFromList(
            ...
            (item, selected) => {
                ...
                return `${marker} ${WHITE}${fname}${RESET}  ${DIM}by ${shortenSig(f.uploader, 4)} - ${when}${RESET}`;
            },
        );
        if (idx === null) return;
        const chosen = items[idx];
        ...
        console.log();
        console.log(
            `  ${DIM}note: this came from a stranger (${shortenSig(chosen.file.uploader, 4)}). scan it after download${RESET}`,
        );
...
const myFilesFlow = async (service: FileShareService, speed: SpeedProfile) => {
    ...
    while (true) {
        ...
        const idx = await selectFromList(
            ...
            (item, selected) => {
                const marker = selected ? `${CYAN}>${RESET}` : " ";
                const sigShort = shortenSig(item.sig, 8);
...
const pickAction = async (sig: string): Promise<"peek" | "download" | "back" | null> => {
    ...
    const idx = await selectFromList(
        `${DIM}tx: ${shortenSig(sig, 10)}${RESET}`,
        actions,
        (item, selected) => {
            const marker = selected ? `${CYAN}>${RESET}` : " ";
...
export const runFileShareMenu = async () => {
    ...
    while (true) {
        ...
        const header = `${FILE_SHARE_LOGO}\n  ${DIM}wallet: ${GREEN}${shortenSig(me, 6)}${RESET}  ${DIM}speed: ${RESET}${currentSpeed}`;
